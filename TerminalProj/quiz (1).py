...
class LiteracyMissionControl(ThemeMixin, AnswerCheckMixin, MissionStateMixin, ctk.CTk):
    ...
    def update_timer(self):
        if self.paused or self.reviewing_previous or not isinstance(self.active_screen, MissionScreen):
            return
        self.active_screen.update_timer(self.timer_seconds)
        if self.timer_seconds <= 0:
            self.record_question_history("", self.current_story()["qs"][self.q_idx][1], "Time's up! Auto-skipping...", "#ff7a7a")
    ...
    def record_question_history(self, user_answer, correct_answer, result_text, result_color):
        story = self.current_story()
        question_text, _ = story["qs"][self.q_idx]
        snapshot = {
            "story": story["story"],
        ...
        self.question_history.append(snapshot)
        self.question_history = self.question_history[-20:]

    def show_previous_question(self):
        if not isinstance(self.active_screen, MissionScreen):
            return
        if not self.question_history:
            self.active_screen.set_feedback("No previous question yet.", "#ffd166")
            return
        self.reviewing_previous = True
        self.cancel_timer()
        self.active_screen.show_previous_review(self.question_history[-1])

    def close_previous_question(self):
        if not isinstance(self.active_screen, MissionScreen):
            return
        self.reviewing_previous = False
        self.active_screen.close_previous_review()
        if not self.paused:
            self.update_timer()
    ...
    def process_answer(self, correct_answer):
        ...
        if self.is_correct_answer(answer, correct_answer):
            ...
            message = f"Incorrect! Correct answer: {accepted}"
            color = "#ff7a7a"
        self.record_question_history(answer, correct_answer, message, color)
    ...
    def skip_question(self):
        if self.paused or not isinstance(self.active_screen, MissionScreen):
            return
        question_answer = self.current_story()["qs"][self.q_idx][1]
        self.record_question_history("", question_answer, "Question skipped.", "#ffd166")
