...
class MenuScreen(BaseScreen):
    def __init__(self, app, theme, username, progress):
        ...
        grade_cards = [
            ("Grade 4", "Forest Sector", "4", self.theme["forest"], self.theme["forest_accent"]),
            ("Grade 5", "Volcano Zone", "5", self.theme["volcano"], self.theme["volcano_accent"]),
            ("Grade 6", "Cyber City", "6", self.theme["cyber"], self.theme["cyber_accent"]),
        ]
        ...
        progress_card = self._menu_card(content, 3, self.theme["card"])
        ...
        self.action_button(progress_card, "Load Progress", app.load_progress).pack(fill="x", padx=28, pady=(0, 10))
        self.action_button(progress_card, "Delete Save", app.delete_save_from_menu, fg="#ff7a7a", hover="#f25f5c", text_color="white").pack(fill="x", padx=28, pady=(0, 24))

    def _menu_card(self, parent, index, color):
        row, column = divmod(index, 2)
        card = self.glass_card(parent, color)
...
class MissionScreen(BaseScreen):
    ...
    def _build_hud(self, parent, state):
        ...
        hud = ctk.CTkFrame(hud_shadow, corner_radius=24, fg_color=self.theme["hud_color"], border_width=1, border_color=self.theme["panel_edge"])
        hud.pack(fill="x")
        ...
        timer_card = ctk.CTkFrame(hud, corner_radius=18, fg_color=self.theme["question_glow"], border_width=1, border_color=self.theme["soft_line"])
